...
public class AudioController : PersistentSingleton<AudioController> {
    ...
    private Dictionary<string, List<int>> lastPlayedSounds = new Dictionary<string, List<int>>();
            ...
            if (variations.Count > 0) {
                ...
                if (noRepeating) {
                    if (!lastPlayedSounds.ContainsKey(soundPrefix)) {
                        lastPlayedSounds.Add(soundPrefix, new List<int>());
                    } else {
                        // If all the variations have been played, clear the list and start over
                        if (lastPlayedSounds[soundPrefix].Count == variations.Count) {
                            // Store the last played index
                            int lastPlayedIndex = lastPlayedSounds[soundPrefix][variations.Count - 1];
                            lastPlayedSounds[soundPrefix].Clear();
                            // Pick a different index for the first sound after clearing the list
                            do {
                    ...
                    while (lastPlayedSounds[soundPrefix].Contains(index) && breakOutCounter < BREAK_OUT_THRESHOLD) {
                        index = Random.Range(0, variations.Count) + 1;
                        breakOutCounter++;
                    }
                    ...
                    // Add the index to the list of played sounds
                    lastPlayedSounds[soundPrefix].Add(index);
                }
